...
export default function ChatPage() {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        setIsUploading(true);
        toast.loading(`Đang tải lên ${selectedFiles.length} file...`, {
          id: "uploading",
        });
        ...
        // Thêm message với file ngay lập tức
      ...
      // Thêm message ngay lập tức vào UI từ response
      // Socket event sẽ bị chặn bởi duplicate check
      if (response && response.id) {
        setMessages((prev) => {
  ...
  const handleUpdateMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === messageId
            ? { ...m, content: oldContent, isEdited: oldIsEdited }
            : m
        )
      );
    } finally {
  ...
  const getTypingText = () => {
    ...
    if (typingMembers.length === 1)
      return `${typingMembers[0].user.fullName} đang nhập...`;
  ...
  const renderMessage = (message) => {
    ...
    return (
      ...
      >
        ...
        >
          {!isOwnMessage && (
            <span className="text-xs font-medium text-primary/70 mb-1.5 truncate max-w-full">
              {sender?.fullName}
            </span>
          )}
          ...
          <div
            className={`relative group rounded-2xl px-4 py-3 min-w-0 max-w-full shadow-sm transition-all hover:shadow-md ${
              isOwnMessage
          ...
          >
            {message.content && (
              <p className="break-words whitespace-pre-wrap word-break leading-relaxed">
                {message.content}
              </p>
            )}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-8 w-8 p-0 hover:bg-background/30 rounded-full backdrop-blur-sm border border-background/20 shadow-sm"
                    >

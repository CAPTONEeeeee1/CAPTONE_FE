...
export default function WorkspacePage() {
  ...
  const fetchMembers = async () => {
    try {
      ...
      const currentMember = lowercaseMembers.find(
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      await workspaceService.inviteMember(workspaceId, {
        email: email.trim(),
        role: selectedRole,
      }); // Giữ nguyên, đã đúng với chuẩn mới

      toast.success(
        `Đã gửi lời mời đến ${email}. Họ cần chấp nhận lời mời để tham gia workspace.`
      );
      setEmail("");
      ...
      const errorMessage =
        error.data?.error || error.message || "Không thể gửi lời mời";
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      ...
      await workspaceService.removeMember(workspaceId, {
      ...
      const errorMessage =
        error.data?.error || error.message || "Không thể xóa thành viên";
      toast.error(errorMessage);
    } finally {
  ...
  const handleChangeRole = async () => {
    ...
    try {
      ...
      await workspaceService.updateMemberRole(workspaceId, {
        memberId: memberToChangeRole.user.id,
        role: newRole,
      }); // Giữ nguyên, đã đúng với chuẩn mới

      setMembers(
        members.map((m) =>
          m.user.id === memberToChangeRole.user.id ? { ...m, role: newRole } : m
        )
      );

      toast.success(
        `Đã cập nhật vai trò của ${memberToChangeRole.user?.fullName} thành ${
          newRole === "leader"
            ? "Leader"
            : newRole === "member"
            ? "Member"
      ...
      const errorMessage =
        error.data?.error || error.message || "Không thể đổi vai trò";
      toast.error(errorMessage);
    } finally {
  ...
  const handleDeleteBoard = async () => {
    ...
    try {
      ...
      toast.success(
        `Board "${boardToDelete.name}" đã được chuyển vào thùng rác. Sẽ tự động xóa sau 15 ngày.`
      );
  ...
  const handleLeaveWorkspace = async () => {
    try {
      ...
      const errorMessage =
        error.data?.error || error.message || "Không thể rời khỏi workspace";
      toast.error(errorMessage);
    } finally {
            ...
            <Alert className="border-primary">
              ...
              <AlertDescription>
                <div className="flex items-center justify-between">
                  <p>
                    Mở khóa không giới hạn board, thành viên và các tính năng
                    cao cấp khác.
                  </p>
                  <Button
                    onClick={() =>
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                ...
                <Badge
                  variant={
                    workspace.visibility === "private" ? "secondary" : "outline"
                  }
                >
                    ...
                    <div className="space-y-4 py-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {selectedRole === "leader" &&
                            "Leader có thể quản lý workspace và mời thành viên mới"}
                          {selectedRole === "member" &&
                            "Member có thể tạo và quản lý boards"}
                        </p>
                      </div>
                    </div>
                    <DialogFooter>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Quản lý quyền truy cập và vai trò
                  </CardDescription>
                </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">
                  Tổ chức công việc theo từng bảng
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <CardContent className="flex flex-col items-center justify-center py-16">
                  ...
                  ["owner", "leader"].includes(currentUserRole) ? (
                    ...
                    <p className="text-muted-foreground text-center">
                      Workspace này chưa có board nào. Chỉ owner hoặc leader mới
                      có thể tạo board.
                    </p>
                  )}
                </CardContent>
                      ...
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            <div
                              className={`h-10 w-10 rounded-lg ${getBoardColor(
                                board.id
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn chuyển board{" "}
              ...
              <br />
              <br />
              Bạn có thể khôi phục hoặc xóa vĩnh viễn board này trong vòng 15
              ngày.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={!!deletingBoardId}>
              Hủy
            </AlertDialogCancel>
            <AlertDialogAction
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <br />
              <br />
              Thành viên này sẽ mất quyền truy cập vào workspace và tất cả
              boards bên trong.
              <span className="text-destructive font-semibold">
                {" "}
                Hành động này có thể được hoàn tác bằng cách mời lại.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-semibold text-foreground">
                {memberToChangeRole?.user?.fullName}
              </span>{" "}
              trong workspace
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {newRole === "leader" &&
                  "Leader có thể quản lý workspace, mời và xóa thành viên"}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <br />
              <br />
              Bạn sẽ mất quyền truy cập vào workspace này và tất cả boards bên
              trong.
              <span className="text-muted-foreground">
                {" "}
                Bạn có thể được mời lại sau nếu muốn.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn xóa vĩnh viễn workspace{" "}
              <span className="font-semibold text-foreground">
                "{workspace?.name}"
              </span>
              ?
              <br />
              <br />
              Tất cả các board, danh sách công việc, và thành viên liên quan sẽ
              bị xóa.
              <span className="text-destructive font-semibold">
                {" "}
